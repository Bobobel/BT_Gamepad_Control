 ...
 * 
 * @JB : Some changes with reference to IDF 5.5 clearing errors in template example:
 * Line 446, 458, 470, 478 : esp_ble_resolve_adv_data new/reduced params
 * Line 1106 : cancelled: bt_cfg.mode = mode;
 * Added a lot of comments
 * 
 * 
 * HID GAP protocol things not documented anywhere else!
 * 
 * Include for this module in ..\include\esp_hid_gap.h
 * 
 */
...
// finds the position of given bda in the results list r
...
#if CONFIG_BT_HID_HOST_ENABLED
/**************************************************
 * Construct a concatenated list of BT scan results
***************************************************/
static void add_bt_scan_result(esp_bd_addr_t bda, esp_bt_cod_t *cod, esp_bt_uuid_t *uuid, uint8_t *name, uint8_t name_len, int rssi)
{
    ...
    // end of list: create new list entry
...
#if CONFIG_BT_BLE_ENABLED
/**************************************************
 * Construct a concatenated list of BLE scan results
***************************************************/
...
#if CONFIG_BT_HID_HOST_ENABLED
/*************************************************************************
 * Prints device properties after reception of ESP_BT_GAP_DISC_RES_EVT
*************************************************************************/
static void handle_bt_device_result(struct disc_res_param *disc_res)
{
    GAP_DBG_PRINTF("BT : " ESP_BD_ADDR_STR, ESP_BD_ADDR_HEX(disc_res->bda));    // 2 concattenated strings, the second for the format
    uint32_t codv = 0;
    ...
    for (int i = 0; i < disc_res->num_prop; i++) {
        ...
        // Main proerties are Extended Inquiry Response, Name, RSSI, COD
        ...
        } else if (prop->type == ESP_BT_GAP_DEV_PROP_COD) {     // COD==Class OF Device
        ...
        // special info in EIR
            ...
            do {    // just once and only one option
                ...
                if (data && len == ESP_UUID_LEN_16) {
                    ...
                    GAP_DBG_PRINTF(", UUID16="); print_uuid(&uuid);
                    break;
                }
                ...
                if (data && len == ESP_UUID_LEN_32) {
                    ...
                    GAP_DBG_PRINTF(", UUID32="); print_uuid(&uuid);
                    break;
                }
                ...
                if (data && len == ESP_UUID_LEN_128) {
                    ...
                    GAP_DBG_PRINTF(", UUID128="); print_uuid(&uuid);
                    break;
                }
            } while(0);
    ...
    // @JB magic: why find_scan_result != NULL?? See above "Some info may come later". find_scan_result handle already existing and new bda
...
#if CONFIG_BT_BLE_ENABLED
/******************************************************************************
 * Prints BLE device properties after reception of ESP_GAP_BLE_SCAN_RESULT_EVT
******************************************************************************/
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    /* V5_5:
    esp_ble_resolve_adv_data_by_type(scan_rst->ble_adv,
...
static void bt_gap_event_handler(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    default:
        ESP_LOGW(TAG, "BT GAP EVENT %d is%s", event, bt_gap_evt_str(event));
        break;
    }
}

/*****************************************
 * Sets security if necessary.
 * Sets pin for legacy pairing.
 * Register the GAP callback.
 * Compare https://github.com/espressif/esp-idf/tree/v5.5/examples/bluetooth/bluedroid/classic_bt/bt_discovery
 */
static esp_err_t init_bt_gap(void)
{
    ...
    // why ESP_BT_NON_DISCOVERABLE and not ESP_BT_GENERAL_DISCOVERABLE like https://github.com/espressif/esp-idf/tree/v5.5/examples/bluetooth/bluedroid/classic_bt/bt_discovery
...
/*******************************************
 * BT scanner by esp_bt_gap_start_discovery
********************************************/
static esp_err_t start_bt_scan(uint32_t seconds)
{
    esp_err_t ret = ESP_OK;
    // start the bluedroid discovery routine
...
#else
/*******************************************
 * lower level: BT init, enable
 * Bluedroid init, enable
 * GAP init for BT and or BLE
 * Compare: https://github.com/espressif/esp-idf/tree/v5.5/examples/bluetooth/bluedroid/classic_bt/bt_discovery
********************************************/
static esp_err_t init_low_level(uint8_t mode)
{
...
#if CONFIG_IDF_TARGET_ESP32
    // @JB not any longer with esp_bt_controller_config_t . Maybe error in template?     bt_cfg.mode = mode;
#endif
#if CONFIG_BT_HID_HOST_ENABLED
...
/***********************************
 * Initialize HID gap service
 * Make checks and creates semaphore.
 * Wrapper for init_low_level
************************************/
...
/**********************************************
 * HID scanning: 
 * scans via BT, BLE/Bluedroid or NIMBLE
 * and collects results in an overall list
***********************************************/
esp_err_t esp_hid_scan(uint32_t seconds, size_t *num_results, esp_hid_scan_result_t **results)
{
    ...
    if (start_bt_scan(seconds) == ESP_OK) {
        WAIT_BT_CB();   // waiting for complete semaphore
    } else {
        return ESP_FAIL;
    }
#endif
